    ...
    <div v-else class="space-y-6">
      ...
      <div class="glass-card p-4 md:p-6">
        ...
        <p v-else class="text-gray-400 text-center py-4">
          No forecast available. Add a monthly income in Settings or enable a fixed auto-save rate.
        </p>
      </div>
    </div>
  </div>
...
const forecast = computed(() => {
  const monthlyIncome = authStore.user?.preferences?.monthlyIncome || 0;
  ...
  return goals.value
    ...
    .map((g) => {
      ...
      if (g.autoSaveEnabled && g.autoSave > 0) {
        ...
        else if (g.frequency === "weekly") monthlyRate *= 4.33;
      }
      ...
      estimatedDate.setMonth(
        estimatedDate.getMonth() + Math.max(1, Math.ceil(monthsNeeded)),
      );
