...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (newGoal.autoSaveEnabled) {
      ...
      const now = new Date();
      if (newGoal.frequency === 'daily') newGoal.nextAutoSave = new Date(now.setDate(now.getDate() + 1));
      else if (newGoal.frequency === 'weekly') newGoal.nextAutoSave = new Date(now.setDate(now.getDate() + 7));
      else if (newGoal.frequency === 'monthly') newGoal.nextAutoSave = new Date(now.setMonth(now.getMonth() + 1));
    }
...
router.post('/:id/add-funds', auth, async (req, res) => {
  try {
    ...
    sendNotification(req.user.id, { type: 'deposit', message: `Funds added to ${goal.title}` });
    ...
    if (goal.saved >= goal.target) {
      ...
      sendNotification(req.user.id, { type: 'goal_completed', message: `Goal completed: ${goal.title}` });
    }
