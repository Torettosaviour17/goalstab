...
router.get('/overview', auth, async (req, res) => {
  try {
    ...
    // Calculate monthly growth (compare this month's total saved vs last month)
    const now = new Date();
    ...
    const transactionsThisMonth = await Transaction.aggregate([
      { $match: { user: mongoose.Types.ObjectId(req.user.id), date: { $gte: firstDayThisMonth } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);
    const savedThisMonth = transactionsThisMonth[0]?.total || 0;

    const transactionsLastMonth = await Transaction.aggregate([
      { $match: { user: mongoose.Types.ObjectId(req.user.id), date: { $gte: firstDayLastMonth, $lte: lastDayLastMonth } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);
    const savedLastMonth = transactionsLastMonth[0]?.total || 0;

    const monthlyGrowth = savedLastMonth ? ((savedThisMonth - savedLastMonth) / savedLastMonth * 100).toFixed(1) : 0;
...
router.get('/trend', auth, async (req, res) => {
  ...
  try {
    const trend = await Transaction.aggregate([
      { $match: { user: mongoose.Types.ObjectId(req.user.id), date: { $gte: startDate } } },
    ...
    const labels = trend.map(item => {
      if (period === 'year') return `${item._id.year}-${item._id.month}`;
      if (period === 'week' || period === 'month') return `${item._id.year}-${item._id.month}-${item._id.day}`;
    });
