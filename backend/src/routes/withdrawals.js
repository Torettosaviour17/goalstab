...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    if (goal.progress < 100) return res.status(400).json({ msg: 'Goal not completed yet' });
    if (amount > goal.saved) return res.status(400).json({ msg: 'Amount exceeds saved balance' });
